<template>
    <div class="max-w-2xl mx-auto mt-8 p-6 bg-white rounded-lg shadow-xl">
        ...
        <div class="space-y-6">
            ...
            <FileUpload v-model:files="files" />
...
const isFormValid = computed(() => {
    ...
    return (isDescriptionValid || files.value.length > 0) && isStatusValid && !fileError.value;
});
...
const hasUnsavedChanges = computed(() => {
    return description.value || status.value || files.value.length > 0;
});
