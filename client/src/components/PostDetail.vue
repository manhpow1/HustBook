<template>
    <div class="max-w-2xl mx-auto mt-8 p-6 bg-white rounded-lg shadow-xl">
        <template v-if="loading">
            <PostSkeleton :isDarkMode="darkMode" avatarSize="w-16 h-16" nameWidth="w-40"
                :contentLines="['w-full', 'w-4/5', 'w-3/5']" :numberOfImages="2" imageHeight="h-64"
                buttonSize="w-24 h-10" commentBoxHeight="h-32" />
        </template>
        ...
        <template v-else-if="post">
            <div class="flex items-center mb-4">
                ...
                <div>
                    ...
                    <p class="text-gray-500 text-sm">{{ formatDate(post.created) }}</p>
                </div>
            </div>
            ...
            <div v-if="post.image && post.image.length > 0" class="mb-4 grid gap-2" :class="gridClass">
                <div v-for="img in post.image" :key="img.id" class="relative overflow-hidden rounded-lg">
                    <img v-lazy="img.url" :alt="post.described" class="w-full h-full object-cover cursor-pointer"
            ...
            <div v-if="post.can_comment === '1'" class="mb-4">
                <MarkdownEditor v-model="newComment" ref="commentInput" :placeholder="$t('writeComment')"
            ...
            <div v-if="comments.length > 0" class="mt-6">
                ...
                <VirtualList class="h-[400px] overflow-auto" :data-key="'id'" :data-sources="comments"
                    :data-component="CommentItem" :estimate-size="70" @tobottom="loadMoreComments">
                    <template #item="{ item }">
